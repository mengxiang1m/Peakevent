...
class ShiftingModule(nn.Module):
    ...
    def forward(self, x, stats1,stats2):
        ...
        # 处理seq_len < pred_len的情况
        if x.shape[1] < self.pred_len:
            # 当历史窗口小于预测窗口时，不计算hist统计量，只使用stats1和stats2
            # 这样保持与初始化时的维度一致：(2 + 0*2)*enc_in = 2*enc_in
            x = torch.cat([stats1, stats2], dim=1)  # B x 2 x E
        else:
            # 原有逻辑：按pred_len分段计算统计量
            num_segments = x.shape[1] // self.pred_len
            hist_mean = torch.zeros([batch_size, num_segments, x.shape[2]]).to(x.device)
            hist_std = torch.zeros([batch_size, num_segments, x.shape[2]]).to(x.device)
            for i in range(0, x.shape[1], self.pred_len):
                if i // self.pred_len < num_segments:
            ...
            hist_stat = torch.cat([hist_mean, hist_std], dim=1)  # B x 2*num_segments x E
            
            # 确保stats1和stats2的维度匹配
            if stats1.dim() == 4:
                stats1 = stats1.squeeze(3)
            if stats2.dim() == 4:
                stats2 = stats2.squeeze(3)
            
            x = torch.cat([hist_stat, stats1, stats2], dim=1)  # B x (2*num_segments+2) x E

        # 确保stats的维度匹配（对于seq_len < pred_len的情况）
        if stats1.dim() == 4:
            stats1 = stats1.squeeze(3)
        if stats2.dim() == 4:
            stats2 = stats2.squeeze(3)
        
        if x.shape[1] == 2:  # 只有stats1和stats2，已经cat过了
            pass
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if configs.busy_decoder:
            ...
            if configs.busy_decoder_modal == 'm':
                # 开启return_indices以获取峰值位置
                self.busy_decoder = torch.nn.MaxPool1d(kernel_size, stride=None, padding=0, dilation=1, return_indices=True, ceil_mode=False)
            else:
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        if self.busy_decoder:
            # 当使用MaxPool1d且return_indices=True时，会返回(values, indices)
            busy_result = self.busy_decoder(dec_out.permute(0,2,1))
            if isinstance(busy_result, tuple):
                # MaxPool返回(values, indices)
                busy_out, busy_indices = busy_result
                busy_out = busy_out.permute(0,2,1)
                busy_indices = busy_indices.permute(0,2,1)
                return dec_out[:, -self.pred_len:, :], busy_out[:,-self.pred_len//24:], busy_indices[:,-self.pred_len//24:]
            else:
                # Linear层只返回values
                busy_out = busy_result.permute(0,2,1)
