...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'long_term_forecast' or self.task_name == 'short_term_forecast' or \
           self.task_name == 'peak_detect_ltf' or self.task_name == 'peak_detect_ltf_basic':
    ...
    def peak_detect_ltf(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        """
        Peak detection task with value prediction + peak classification
        """
        ...
        # Peak decoder (独立的 peak 分类器)
        peak_dec_input = self.dec_embedding(x_dec, x_mark_dec)
        peak_out = self.peak_decoder(peak_dec_input, enc_out, x_mask=None, cross_mask=None)
        
        return dec_out, peak_out
    
    def peak_detect_ltf_basic(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        """
        Peak detection basic task: only value prediction
        """
        # Normalization from Non-stationary Transformer
        means = x_enc.mean(1, keepdim=True).detach()
        x_enc = x_enc - means
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        if self.task_name == 'peak_detect_ltf_basic':
            dec_out = self.peak_detect_ltf_basic(x_enc, x_mark_enc, x_dec, x_mark_dec)
            return dec_out[:, -self.pred_len:, :]  # [B, L, D]
        if self.task_name == 'peak_detect_ltf':
            dec_out, peak_out = self.peak_detect_ltf(x_enc, x_mark_enc, x_dec, x_mark_dec)
            return dec_out[:, -self.pred_len:, :], peak_out[:, -self.pred_len:, :]  # [B, L, D], [B, L, D]
        if self.task_name == 'imputation':
