...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if self.task_name == 'peak_detect_ltf':
            self.peak_predict = nn.Sequential(
                nn.Dropout(self.dropout),
                nn.Linear(self.d_model, self.seg_len)
            )
    ...
    def encoder(self, x, return_peak=False):
        ...
        # Peak分类预测（如果需要）
        if return_peak and hasattr(self, 'peak_predict'):
            # 1,bcm,d -> 1,bcm,w -> b,c,s
            peak_y = self.peak_predict(hy).view(-1, self.enc_in, self.pred_len)
            # permute but no denorm for peak logits
            peak_y = peak_y.permute(0, 2, 1)
            return y, peak_y
    ...
    def imputation(self, x_enc):
        # Encoder
        return self.encoder(x_enc, return_peak=False)
