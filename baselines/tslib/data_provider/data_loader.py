...
try:
    ...
    def hf_hub_download(*args, **kwargs):
        raise ImportError(
            "huggingface_hub is not installed and local fallback data was not found."
        )
...
def _load_hf_dataset(cfg_name):
    if load_dataset is None:
        raise ImportError(
            "datasets is not installed. Please provide local csv files for comparison experiments."
        )
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if os.path.exists(local_fp):
            ...
            df_raw = _load_hf_dataset(cfg_name)
...
def _read_stsep_split(cfg_path, dataset_name, split_name):
    with open(cfg_path, 'r', encoding='utf-8') as f:
        cfg = json.load(f)
    datasets_cfg = cfg.get('datasets', {})
    if dataset_name not in datasets_cfg:
        raise KeyError(
            f'dataset_name={dataset_name} not found in dataset config: {cfg_path}'
        )
    ds_cfg = datasets_cfg[dataset_name]
    splits = ds_cfg.get('splits', {})
    if split_name not in splits:
        raise KeyError(
            f'split={split_name} not found for dataset={dataset_name} in {cfg_path}'
        )
    return ds_cfg, splits


class Dataset_STSEP(Dataset):
    """
    Dataset for STSEP comparison experiments.
    Uses fixed split indices from dataset_configs.json to match PatchEvent splits.
    """
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='wlel.csv',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        self.args = args
        if size is None:
            self.seq_len = 96
        ...
        self.dataset_name = getattr(args, 'dataset_name', '') or os.path.splitext(
            os.path.basename(self.data_path)
        )[0]
        self.window_stride = max(1, int(getattr(args, 'window_stride', 1)))
        self.__read_data__()

    def __read_data__(self):
        if not self.dataset_config:
            raise ValueError('--dataset_config is required when --data stsep')
        self.scaler = StandardScaler()

        ds_cfg, splits = _read_stsep_split(self.dataset_config, self.dataset_name, self.flag)
        split_cfg = splits[self.flag]
        train_cfg = splits['train']
        border1 = int(split_cfg['start'])
        border2 = int(split_cfg['end']) + 1
        train_b1 = int(train_cfg['start'])
        train_b2 = int(train_cfg['end']) + 1
        ...
        if 'date' not in df_raw.columns:
            raise KeyError(f'Expected `date` column in {csv_path}')
        if self.target not in df_raw.columns:
            raise KeyError(f'Expected target column `{self.target}` in {csv_path}')
        ...
        if self.scale:
            train_data = df_data.iloc[train_b1:train_b2]
            self.scaler.fit(train_data.values)
            data = self.scaler.transform(df_data.values)
        else:
            data = df_data.values
        ...
        max_start = border2 - self.seq_len - self.pred_len
        if max_start < border1:
            self.starts_global = np.array([], dtype=np.int64)
            self.starts_local = np.array([], dtype=np.int64)
        else:
            offset = (self.window_stride - (border1 % self.window_stride)) % self.window_stride
            first_aligned = border1 + offset
            self.starts_global = np.arange(
                first_aligned, max_start + 1, self.window_stride, dtype=np.int64
            )
            self.starts_local = self.starts_global - border1
...
class UEAloader(Dataset):
    ...
    def load_single(self, filepath):
        if load_from_tsfile_to_dataframe is None:
            raise ImportError(
                "sktime is required for UEA classification datasets."
            )
