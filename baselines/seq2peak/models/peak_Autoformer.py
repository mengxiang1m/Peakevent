...
class Projectorh(nn.Module):
    ...
    def forward(self, x, stats):
        ...
        x = self.feature_concentration(x).squeeze(-1)
        y = self.backbone(x)      # B x O
...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,
        ...
        dec_zeros = torch.zeros_like(x_dec[:, -self.pred_len:, :])
        x_dec_new = torch.cat([x_enc[:, -self.label_len:, :], dec_zeros], dim=1).to(x_enc.device).clone()
        # Model Inference
        ...
        if self.hour_day=='h':
            if self.with_shift != 'ws':
                hour_repeat = (dec_out.shape[1] + 23) // 24
                std_enc_hour = std_enc_hour.repeat(1, hour_repeat, 1, 1).reshape(dec_out.shape[0], -1, dec_out.shape[2])[:, :dec_out.shape[1], :]
                mean_enc_hour = mean_enc_hour.repeat(1, hour_repeat, 1, 1).reshape(dec_out.shape[0], -1, dec_out.shape[2])[:, :dec_out.shape[1], :]
            else:
