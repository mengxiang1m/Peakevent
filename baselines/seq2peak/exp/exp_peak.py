    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if hasattr(test_data, 'starts_global'):
                    bs = pred.shape[0]
                    batch_starts = test_data.starts_global[start_cursor:start_cursor + bs]
                    start_cursor += bs
                    window_starts.extend(batch_starts.tolist())
                if i % 10 == 0:
        ...
        np.save(folder_path + 'metrics.npy', np.array([mae, mse, rmse, mape, mspe, mae_b, mse_b, rmse_b, mape_b, mspe_b]))
        np.save(folder_path + 'pred.npy', preds)
        np.save(folder_path + 'true.npy', trues)
        if preds.shape[-1] == 1:
            per_window_preds = preds[:, :, 0]
        else:
            per_window_preds = preds[:, :, -1]
        np.save(folder_path + 'per_window_preds.npy', per_window_preds.astype(np.float32))
        if window_starts:
            np.save(folder_path + 'per_window_starts.npy', np.array(window_starts, dtype=np.int64))
