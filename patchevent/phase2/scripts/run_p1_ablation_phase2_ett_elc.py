"""
Run Phase-2 G0_full using Phase-1 ablated encoders for ETT and ELC.
Step 2 of Phase1 ablation: uses the ablated encoders trained by
patchevent/phase1/run_p1_ablation_ett_elc.py
"""
...
def main():
    ...
    for domain, paths in DOMAINS.items():
        p1_base = f"patchevent/phase1/checkpoints/{domain}_ablation"
        out_base = f"patchevent/phase2/checkpoints/{domain}_p1_ablation"
        for gp_name, p1_name in GROUPS.items():
            for seed in SEEDS:
                ...
                if os.path.exists(os.path.join(out_dir, "test_summary.json")):
                    print(f"[SKIP] {domain}/{gp_name}_s{seed}")
                    skip += 1
                    continue
                enc_ckpt = os.path.join(p1_base, p1_name, f"s{seed}", "best_model.pth")
                if not os.path.exists(enc_ckpt):
                    print(f"[FAIL] missing encoder: {enc_ckpt}")
                    fail += 1
                    continue
                cmd = [
                    PYTHON, P2_TRAIN,
                    "--encoder_ckpt", enc_ckpt,
                    "--series_path", paths["series"],
                    "--events_path", paths["events"],
                    "--output_dir", out_dir,
                    "--seed", str(seed),
                ] + list(COMMON)
                print(f"\n{'='*64}")
                print(f"[START] {domain}/{gp_name}_s{seed}")
                print(f"  encoder: {enc_ckpt}")
                print(f"  output : {out_dir}")
                print(f"{'='*64}")
                ret = subprocess.run(cmd, cwd=os.getcwd())
                if ret.returncode != 0:
                    print(f"[FAIL] {domain}/{gp_name}_s{seed}")
                    fail += 1
                else:
                    done += 1

    print(f"\n{'='*64}")
    print(f"P1->P2 ablation ETT+ELC: total={total} done={done} skip={skip} fail={fail}")
    print(f"{'='*64}")
