"""
Build Electricity (ELC) event-labeled dataset.
Follows the same format as ETT/WLEL event datasets.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build ELC event dataset")
    parser.add_argument(
        "--input-file",
        type=Path,
        default=Path("dataset/electricity/intermediate/electricity_mixed_with_peaks_lookahead_5.csv"),
        help="Source electricity dataset CSV",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("dataset/electricity/event_v1"),
...
def main() -> None:
    ...
    series_path = data_dir / f"elc_event_series_{config.version}.csv"
    ...
    patch_path = data_dir / f"elc_patch_labels_{config.version}.csv"
    ...
    save_events(
        ...
        dataset_prefix="elc",
        default_event_source="gradient_width",
    )
    ...
    report_md_path.write_text(
        report_to_markdown(quality_report, dataset_name="ELC"),
        encoding="utf-8",
    )

    print("=" * 60)
    print("ELC event dataset build completed")
    print(f"Output dir: {output_dir}")
    print(f"  - Point-level CSV: data/{series_path.name}")
    print(f"  - Patch-level labels: data/{patch_path.name}")
    print(f"  - Events JSONL: data/elc_events_{config.version}.jsonl")
