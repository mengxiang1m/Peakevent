"""
ETT ablation with optimal weight w=1 (uniform: 1.0,0.5,1.0,0.5)
Architecture + AR + P1 ablation, all × 3 seeds
"""
import sys
import subprocess
import os
...
SEEDS = [42, 123, 456]
BASE_OUT = "patchevent/phase2/checkpoints/ett_optimal_ablation"

ENCODER_BASE = "patchevent/phase1/checkpoints/ett"
P1_ABLATION_BASE = "patchevent/phase1/checkpoints/ett_ablation"
SERIES = "dataset/ett/event_v1/data/ett_event_series_v1.csv"
EVENTS = "dataset/ett/event_v1/data/ett_events_v1.jsonl"

# ETT optimal: w=1 (uniform)
OPTIMAL_WEIGHTS = "1.0,0.5,1.0,0.5"
...
def run_one(cfg_name, extra_args, seed):
    ...
    # For P1 ablation configs, use ablated encoder
    ...
    if not os.path.exists(encoder):
        print(f"    SKIP {cfg_name}_s{seed}: encoder not found")
        return None
...
def main():
    ...
    print("=== ETT Architecture Ablation (w=1, optimal) ===")
    arch_cfgs = {
    ...
    for name, args in arch_cfgs.items():
        ...
        r = run_3seed(name, args)
        print(r)

    # AR ablation
    print("\n=== ETT AR Ablation (w=1, optimal) ===")
    ar_cfgs = {
    ...
    # GD3 uses NO attr weights (that's the ablation)
    for name, args in ar_cfgs.items():
        extra = list(args)
        if name == "GD3_no_attr_weight":
            # Remove attr_loss_weights from COMMON by not using OPTIMAL_WEIGHTS
            # We need a special run without --attr_loss_weights
            pass  # Actually COMMON always includes it; GD3 should use uniform 1.0 which IS the optimal for ETT
            # So GD3 with w=1 is same as Full... This is a special case for ETT.
            # For ETT w=1, "w/o AttrWeight" means truly uniform (1,1,1,1) not (1,0.5,1,0.5)
        print(f"  {name}: ", end="", flush=True)
        r = run_3seed(name, extra)
        print(r)

    # P1 ablation
    print("\n=== ETT P1 Ablation (w=1, optimal) ===")
    p1_cfgs = ["GP1a_wo_has_apex", "GP1b_wo_distance", "GP1c_wo_phase",
               "GP1d_wo_offset", "GP1e_generic_npp"]
    base_args = ["--encoder_mode", "frozen", "--unfreeze_last_n", "1", "--use_memory_pos", "--use_self_attn_agg"]
    for name in p1_cfgs:
        print(f"  {name}: ", end="", flush=True)
        r = run_3seed(name, base_args)
        print(r)
